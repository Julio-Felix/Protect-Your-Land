...
export default class GameScene extends Phaser.Scene {
  ...
  PlayerAttack(player, monster){
    if(!monster.already_attack){
      ...
      monster.body.setVelocityY(-400)
      if (monster.anims.getCurrentKey() == "right_slime") monster.body.velocity.x = -400;
      if (monster.anims.getCurrentKey() == "left_slime") monster.body.velocity.x = 400;
  ...
  SlimeMovement(slime){
    if(slime.active){
      ...
      if(slime.body.blocked.left) slime.movement_ac=200
  
      if(slime.body.blocked.right) slime.movement_ac=-200
      
      if(slime.movement_ac > 0) slime.anims.play('right_slime',true) 
      else slime.anims.play('left_slime',true)

      if(slime.body.blocked.down && !slime.already_attack) slime.body.setVelocityX(slime.movement_ac)
    }
